...
const EditSession: React.FC<EditSessionProps> = ({ session, onFinish }) => {
    ...
    return (
        <>
            <div className="pb-40">
                <header className="sticky top-0 z-20 p-4 bg-background/90 backdrop-blur-sm border-b border-surface">
                    <div className="flex justify-between items-center max-w-3xl mx-auto">
                        <h1 className="text-2xl font-bold truncate pr-2">Edit Session</h1>
                        <input type="date" value={sessionDate} onChange={e => setSessionDate(e.target.value)} className="bg-surface text-text-primary px-3 py-1 rounded-full text-md"/>
                    </div>
                </header>
                ...
                <main className="px-4 space-y-4 mt-4 max-w-3xl mx-auto">
                    {editedExercises.map((pEx, exIndex) => (
                        <div key={pEx.exerciseId + exIndex} data-drag-item="true" draggable onDragStart={(e) => handleDragStart(e, exIndex)} onDragOver={(e) => e.preventDefault()} onDragEnter={() => handleDragEnter(exIndex)} onDragLeave={() => setDragOverIndex(null)} onDrop={() => handleDrop(exIndex)} onDragEnd={cleanupDragState}
                            className={`bg-surface p-4 rounded-lg shadow-md transition-all duration-200 ${draggedIndex === exIndex ? 'opacity-50 shadow-2xl scale-105' : ''} ${dragOverIndex === exIndex && dragOverIndex !== draggedIndex ? 'outline-2 outline-dashed outline-primary -outline-offset-2' : ''}`}>
                            <div className="flex justify-between items-center mb-3">
                                <div className="flex items-center gap-3">
                                    <div onTouchStart={(e) => handleTouchStart(e, exIndex)} className="cursor-grab p-2 -ml-2"><DragHandleIcon className="w-5 h-5 text-text-secondary flex-shrink-0" /></div>
                                    <h3 className="text-lg font-bold">{pEx.exerciseName}</h3>
                                </div>
                                <button onClick={() => removeExercise(exIndex)} className="text-red-400 hover:text-red-500 transition-colors text-sm">Remove</button>
                            </div>
                            {pEx.sets.map((set, setIndex) => (
                                <div key={set.id} className="flex items-center gap-2 mb-2">
                                    ...
                                    <input type="number" min="0" max="1000" value={set.weight} onChange={e => updateSet(exIndex, setIndex, 'weight', e.target.value)} onFocus={handleFocus} className="w-full bg-background p-2 rounded text-center" placeholder="kg" />
                                    <span className="text-text-secondary">kg</span>
                                    <input type="number" min="1" max="50" value={set.reps} onChange={e => updateSet(exIndex, setIndex, 'reps', e.target.value)} onFocus={handleFocus} className="w-full bg-background p-2 rounded text-center" placeholder="Reps" />
                                    <span className="text-text-secondary">reps</span>
                                    <button onClick={() => removeSet(exIndex, setIndex)} className="text-red-500 text-2xl font-light hover:text-red-400 w-8 text-center">×</button>
                                </div>
                            ))}
                            <button onClick={() => addSet(exIndex)} className="w-full bg-primary/20 text-primary p-2 rounded-lg mt-2 font-semibold hover:bg-primary/30 transition-colors text-sm">Add Set</button>
                        </div>
                    ))}
                </main>
            </div>
            ...
            <footer className="fixed bottom-0 left-0 right-0 p-4 bg-background/90 backdrop-blur-sm border-t border-surface z-10">
                <div className="space-y-3 max-w-3xl mx-auto">
                    <button onClick={() => setIsPickerOpen(true)} className="w-full bg-secondary/20 text-secondary p-3 rounded-lg font-semibold hover:bg-secondary/30 transition-colors">+ Add Exercise</button>
                    <div className="flex gap-4">
                         <button onClick={() => onFinish()} className="w-full bg-surface p-3 rounded-lg hover:bg-surface/80 transition-colors">Cancel</button>
                         <button onClick={handleSaveChanges} className="w-full bg-primary p-3 rounded-lg font-bold hover:bg-primary/90 transition-colors">Save Changes</button>
                    </div>
                </div>
            </footer>
                         ...
                         <div className="flex-none">
                             ...
                             <div className="flex space-x-2 overflow-x-auto pb-2 -mx-4 px-4">
                                 {['all', ...Object.values(Equipment)].map(eq => (
                                     <button key={eq} onClick={() => setEquipmentFilter(eq as Equipment | 'all')} className={`px-4 py-2 text-sm font-semibold rounded-full whitespace-nowrap transition-colors ${equipmentFilter === eq ? 'bg-primary text-white' : 'bg-surface text-text-secondary hover:bg-gray-700'}`}>{eq.charAt(0).toUpperCase() + eq.slice(1)}</button>
                                 ))}
                             </div>
                         </div>
