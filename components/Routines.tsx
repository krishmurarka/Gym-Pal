        ...
        return (
            <div className="p-4 max-w-4xl mx-auto">
                ...
                <input
                    ...
                    className={`w-full bg-input p-3 rounded-xl mb-4 text-lg text-text-primary placeholder-text-secondary border transition-all ${isNameInvalid ? 'border-danger' : 'border-border focus:border-secondary'}`}
                />

                <div className="flex items-center bg-surface p-1 rounded-full mb-4 overflow-x-auto">
                    {isEditing.variants.map((variant, index) => (
                        <div key={variant.id} className="flex items-center flex-shrink-0">
                            <button
                                onClick={() => setActiveVariantIndex(index)}
                                className={`px-4 py-1.5 font-semibold whitespace-nowrap transition-colors rounded-full text-sm ${activeVariantIndex === index ? 'bg-primary text-background' : 'text-text-secondary hover:text-text-primary'}`}
                            >
                                Variant {variant.name}
                            </button>
                            {isEditing.variants.length > 1 && (
                                <button onClick={() => deleteVariant(index)} className="ml-1 text-text-secondary hover:text-danger p-1">
                                    <TrashIcon className="w-4 h-4" />
                                </button>
                            )}
                        </div>
                    ))}
                    <button onClick={addVariant} className="p-2 text-text-secondary hover:text-primary ml-1">
                    ...
                    {currentExercises.map((exConfig, exIndex) => {
                        ...
                        return (
                            <div 
                                ...
                                className={`bg-surface border border-border p-4 rounded-3xl space-y-3 shadow-md transition-all duration-200 
                                ...
                                <div className="flex justify-between items-center">
                                    <div className="flex items-center gap-3">
                                        <div onTouchStart={(e) => handleTouchStart(e, exIndex)} className="cursor-grab p-2 -ml-2">
                                            <DragHandleIcon className="w-6 h-6 text-text-secondary flex-shrink-0" />
                                        </div>
                                        <div>
                                    ...
                                    <button onClick={() => removeExerciseFromRoutine(exConfig.exerciseId)} className="text-danger hover:brightness-125 transition-all text-sm font-semibold">Remove</button>
                                </div>
                                {exConfig.sets.map((set, setIndex) => (
                                    <div key={set.id} className="flex items-center gap-2">
                                        <span className="font-bold text-text-secondary w-8 text-center">{setIndex + 1}</span>
                                        <input type="number" min="0" max="1000" value={set.weight} onChange={e => handleSetChange(exIndex, setIndex, 'weight', e.target.value)} onFocus={handleFocus} className="w-full bg-input border border-border p-2 rounded-xl text-center" placeholder="kg" />
                                        <span className="text-text-secondary text-sm">kg</span>
                                        <input type="number" min="1" max="50" value={set.reps} onChange={e => handleSetChange(exIndex, setIndex, 'reps', e.target.value)} onFocus={handleFocus} className="w-full bg-input border border-border p-2 rounded-xl text-center" placeholder="Reps" />
                                        <span className="text-text-secondary text-sm">reps</span>
                                        <button onClick={() => removeSetFromExercise(exIndex, setIndex)} className="text-danger text-2xl font-light hover:brightness-125 w-8 text-center">×</button>
                                    </div>
                                ))}
                                <button onClick={() => addSetToExercise(exIndex)} className="w-full bg-secondary/20 text-secondary p-2 rounded-xl mt-2 font-semibold hover:bg-secondary/30 transition-colors text-sm">Add Set</button>
                            </div>
                        );
                    })}
                </div>

                <button onClick={() => setIsPickerOpen(true)} className="w-full border-2 border-dashed border-border text-text-secondary p-3 rounded-xl mb-4 font-semibold hover:bg-surface hover:border-solid transition-all">Add Exercise to '{isEditing.variants[activeVariantIndex]?.name}'</button>
                <div className="flex gap-4">
                    <button onClick={() => setIsEditing(null)} className="w-full btn btn-ghost">Cancel</button>
                    <button onClick={handleSaveRoutine} className="w-full btn btn-primary">Save Routine</button>
                </div>
                    ...
                    <div className="fixed inset-0 bg-background/95 backdrop-blur-sm flex flex-col p-4 z-50">
                        {isCreatingExercise ? (
                            <div className="w-full max-w-md mx-auto mt-10">
                                ...
                                <input type="text" placeholder="Exercise Name" value={newExercise.name} onChange={e => setNewExercise({...newExercise, name: e.target.value})} className="w-full bg-input border border-border p-3 rounded-xl mb-4"/>
                                <select value={newExercise.muscleGroup} onChange={e => setNewExercise({...newExercise, muscleGroup: e.target.value as MuscleGroup})} className="w-full bg-input border border-border p-3 rounded-xl mb-4">
                                    {Object.values(MuscleGroup).map(group => <option key={group} value={group}>{group}</option>)}
                                </select>
                                <select value={newExercise.equipment} onChange={e => setNewExercise({...newExercise, equipment: e.target.value as Equipment})} className="w-full bg-input border border-border p-3 rounded-xl mb-6">
                            ...
                            <>
                                <div className="flex-none">
                                    ...
                                    <input type="text" placeholder="Search exercises..." value={exerciseSearch} onChange={(e) => setExerciseSearch(e.target.value)} className="w-full bg-surface border border-border p-3 rounded-xl my-4"/>
                                    <div className="flex space-x-2 overflow-x-auto pb-2 -mx-4 px-4">
                                        {(['all', ...Object.values(Equipment)] as const).map(eq => (
                                            <button key={eq} onClick={() => setEquipmentFilter(eq)} className={`px-4 py-2 text-sm font-semibold rounded-full whitespace-nowrap transition-colors ${equipmentFilter === eq ? 'bg-secondary text-white' : 'bg-surface text-text-secondary hover:bg-border'}`}>{eq.charAt(0).toUpperCase() + eq.slice(1)}</button>
                                        ))}
                                    </div>
                                </div>
                                <div className="flex-1 overflow-y-auto mt-4">
                                    <div className="sticky top-0 z-20 bg-background/95 py-3"><button onClick={() => setIsCreatingExercise(true)} className="w-full bg-primary/20 text-primary p-3 rounded-xl font-semibold hover:bg-primary/30 transition-colors">+ Add Custom Exercise</button></div>
                                    ...
                                    {Object.entries(groupedExercises).map(([group, exercises]) => (
                                        <div key={group} className="mb-4">
                                            ...
                                            {exercises.map(ex => <button key={ex.id} onClick={() => addExerciseToRoutine(ex)} className="w-full text-left bg-surface border border-border p-3 rounded-xl mb-2 hover:bg-border transition-colors">{ex.name}</button>)}
                                        </div>
                                    ))}
                                </div>
